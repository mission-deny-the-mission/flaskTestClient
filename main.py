...
if response.status_code == 201:
    response = requests.get("http://{0}/CompileMDtoHTML/{1}/example.md".format(address, dir))
    file = open("example.html", "w")
    file.write(response.text)
    file.close()

    response = requests.get("http://{0}/CompileMDtoPDF/{1}/example.md".format(address, dir))
    ...
    requests.get("http://{0}/Delete/{1}".format(address, dir))
